...
{ 
    ...
    cout << largestTwinPrime(100,108)  <<endl;
    cout <<  largestTwinPrime(1,8) <<endl;
    cout <<  largestTwinPrime(20,59) <<endl;
    

}
                            //function defination (declare)

int largestTwinPrime(int a, int b){    //Keyword largest range a and b (lower bound and upper bound)
                                       //go from the upper bound cause most likely what you are looking for close to the upper bound 
                                       // the first twin prime you found should be the biggest countPrimes
            for (int i=b; i>=a;  i--) {  //go b to a
                    if(isTwinPrime(i))
                            return i;   //return biggest twin prime
            }
            return -1;     //if you didnt find anything prime inside the for loop return -1
